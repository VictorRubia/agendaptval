...
import 'package:agendaptval/vistas/tareasProf/revisar_tarea_prof_widget.dart';
...
class ListaTareasProfRevisionWidget extends StatefulWidget {
  const ListaTareasProfRevisionWidget({Key key}) : super(key: key);



  @override
  _ListaTareasProfRevisionWidgetState createState() => _ListaTareasProfRevisionWidgetState();
}

class _ListaTareasProfRevisionWidgetState extends StateMVC {
  ...
  _ListaTareasProfRevisionWidgetState(): super(Controller()){
    _con = Controller.con;
  }
  Controller _con;
  ...
  void initState() {
    ...
    // initial load
    tareas = _con.getTareasDeTuteladosRevisionProfesor();
                                    ...
                                    child: ListTile(
                                          ...
                                          IconButton(
                                            ...
                                            onPressed: () async{
                                              await Navigator.push(
                                                context,
                                                MaterialPageRoute(
                                                  builder: (context) => RevisarTareaProfWidget(_searchResult[index],getAlumno(_searchResult[index].idUsuarioRealiza,listaUsuarios)),
                                                ),
                                              ).then((_) => _onRefresh());
                                            },
                                          ),
                                          IconButton(
                                            ...
                                            onPressed: () async{
                                              await Navigator.push(
                                                context,
                                                MaterialPageRoute(
                                                  builder: (context) => EditarTareaProfWidget(_searchResult[index],getAlumno(_searchResult[index].idUsuarioRealiza,listaUsuarios)),
                                                ),
                                              ).then((_) => _onRefresh());
                                            },
                                          ),
                                      ...
                                      onTap: () async{
                                        await Navigator.push(
                                          context,
                                          MaterialPageRoute(
                                            builder: (context) => DescripcionTareaProfWidget(_searchResult[index].idTarea, getAlumno(_searchResult[index].idUsuarioRealiza, listaUsuarios)),
                                          ),
                                        );
                                      },
                                    ),
                            ...
                            Expanded(
                              child: Center(
                                child: ListView(
                                  shrinkWrap: true,
                                  children: [
                                    Icon(Icons.done_outline, size: 100, color: FlutterFlowTheme.laurelGreenDarker,),
                                    SizedBox(height: 25,),
                                    Container(
                                      alignment: Alignment.center,
                                      child: searchString.isNotEmpty ? Text('No hay tareas pendientes de revisar según su búsqueda', style: FlutterFlowTheme.title3, textAlign: TextAlign.center) : Text('No hay tareas pendientes de revisar', style: FlutterFlowTheme.title3) ,
